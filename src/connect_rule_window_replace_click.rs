...
pub fn connect_rule_window_replace_click(gui_data: &GuiData) {
    ...
    check_button_replace_name.connect_toggled(move |_e| {
        update_examples(&window_rules, None);
    });
    let window_rules = gui_data.window_rules.clone();
    check_button_replace_extension.connect_toggled(move |_e| {
        update_examples(&window_rules, None);
    });
    let window_rules = gui_data.window_rules.clone();
    check_button_replace_both.connect_toggled(move |_e| {
        update_examples(&window_rules, None);
    });

    let window_rules = gui_data.window_rules.clone();
    check_button_replace_case_sensitive.connect_toggled(move |_e| {
        update_examples(&window_rules, None);
    });
    let window_rules = gui_data.window_rules.clone();
    check_button_replace_case_insensitive.connect_toggled(move |_e| {
        update_examples(&window_rules, None);
    });
