...
tree_view_bottom_usage_name = 用法名稱
...
upper_files_folders_label_update = 檔案/資料夾({ $files_number }) - ##### UPDATE REQUIRED #####
upper_files_folders_label_up_to_date = 檔案/資料夾({ $files_number }) - up to date
# Select popover
...
# Un/Select custom
select_custom_example = 用法：*/folder-nr*/* 或 name-version-*.txt
...
label_custom_instruction =
    ...
    $(PARENT) - 列印父資料夾名稱
    $(N)/$(K) - 列印數字(參數是可選的)
    $(N:3:4:5) 列印數字從3, 列印數字。 帶第4步
            並填充零到5個位置。
    	K只是列表中的位置，也使用資料夾中的位置項。
...
label_number_step = 步驟
label_number_fill_zeros = 用零填充
# Add text
check_button_add_text_before_name = 基礎檔名之前
check_button_add_text_after_name = 基礎檔名之後
label_add_text = 要新增的文字：
# Replace
check_button_replace_name = 僅基礎檔名
check_button_replace_extension = 僅副檔名
check_button_replace_both = 兩者都是
check_button_replace_case_sensitive = 區分大小寫
check_button_replace_case_insensitive = 不敏感大小寫
check_button_replace_regex = 使用正則表達式
check_button_replace_replace_all = 取代所有事件
label_replace_replacing_strings = 取代字串：
label_replace_text_to_find = 要查找的文字
label_replace_text_to_replace = 取代的文字
label_replace_captures = 抓取
label_replace_captured_captures = 捕獲的抓取
label_replace_captures_number = ({ $capture_number } captures)
label_replace_no_captures = 沒有抓取
label_replace_invalid_regex = 無效的憑單
# Trim
...
check_button_trim_case_insensitive = 不敏感大小寫
...
rule_description_zeros = 並以 { $zeros } 零填充，
rule_description_step = 從 { $start } 開始，步驟為 { $step }{ $zeros }
rule_description_lowercase = 小寫
rule_description_uppercase = 大寫
rule_description_text = 文字
rule_description_added_text = 新增文字：
...
rule_description_trimming = 正在從{ $trim_text }“從 { $where_remove } 中嘗試” { $trim_text }
rule_description_custom_rule = 自訂規則： { $custom_rule }
rule_description_replace = 正在取代 { $additional_regex_text } "{ $text_to_find }" 為 "{ $text_to_replace }"
# Notebooks
notebook_tab_custom = 自訂
notebook_tab_case_size = 大寫/小寫字母
notebook_tab_purge = 清理
notebook_tab_add_number = 新增數字
notebook_tab_add_text = 新增文字
